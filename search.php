...
/* Ici on va faire 1 requête sql en fonction de la demande de l'utilisateur, le resultat final sera afficher dans tout les cas */

$user = new login($_SESSION['login'], $_SESSION['mdp']);
$checkUtilisateurActuel = $user->getInfo($_SESSION['login']);

$result = "";
$tweet = $_GET['search'];
if ($_GET['type'] == 'Tweet') {
    $queryTweet = $user->basicQuery('SELECT pseudo, post_content FROM users INNER JOIN post ON users.id = post.id_user where post_content like "%'.$_GET['search'].'%" order by id_post desc limit 20');
    while ($speed = $queryTweet->fetch()) {
        $result .= "<strong>".$speed['pseudo']."</strong>: ".$speed['post_content']."<br>";
    }
}

if ($_GET['type'] == "User") {
    $queryTweet = $user->basicQuery('select * from users INNER JOIN user_info ON users.id = user_info.id_user where pseudo like "%'.$_GET['search'].'%" limit 20');
    while ($speed = $queryTweet->fetch()) {
        $result .= '<div class="lukfaislefrontstp">Pseudo: <a href="membre.php?pseudo=' . $speed['pseudo'] . '">'.$speed['pseudo'].'</a></div><br>';
    }
}
if($result == ""){
    $result = "Désolé votre recherche n'a pas aboutie";
}

if($_GET['search'] == ""){
    $result = "Merci de saisir quelque chose ";
}
...
<body>
    ...
    <div class ="result">
      <div class="titre"><h1>Resultat en rapport avec <strong><?php echo $tweet ?></strong></h1></div>
      <?php echo $result ?>
    </div>
</body>
</html>
